...
class GeneratorNetwork(nn.Module):
    ...
    """
    def __init__(self, x_dim, v_dim, r_dim=256, z_dim=64, h_dim=128, L=12):
        ...
        self.upsample = nn.ConvTranspose2d(h_dim, h_dim, kernel_size=2, stride=2, padding=0)
        self.downsample = nn.Conv2d(x_dim, x_dim, kernel_size=2, stride=2, padding=0)
    ...
    def forward(self, x, v, r):
        ...
        # Increase dimensions
        r = r.view(batch_size, -1, 1, 1).repeat(1, 1, h//2, w//2)
        v = v.view(batch_size, -1, 1, 1).repeat(1, 1, h//2, w//2)

        # Reset hidden state
        hidden_g = x.new_zeros((batch_size, self.h_dim, h//2, w//2))
        hidden_i = x.new_zeros((batch_size, self.h_dim, h//2, w//2))

        # Reset cell state
        cell_g = x.new_zeros((batch_size, self.h_dim, h//2, w//2))
        cell_i = x.new_zeros((batch_size, self.h_dim, h//2, w//2))
        ...
        for _ in range(self.L):
            ...
            q_mu, q_log_std = torch.split(o, self.z_dim, dim=1)
            posterior_distribution = Normal(q_mu, torch.exp(q_log_std))
    ...
    def sample(self, x_shape, v, r):
        ...
        hidden_g = v.new_zeros((batch_size, self.h_dim, h//2, w//2))
        cell_g = v.new_zeros((batch_size, self.h_dim, h//2, w//2))
